...
const iftttWebhookSendInit: RequestInit = {
	headers: {
		...
		"User-Agent": `Deno/${Deno.version.deno}-${Deno.build.os}-${Deno.build.arch} SendIFTTTWebhook/0.1.0`
	},
	method: "POST",
	redirect: "follow"
};
...
class IFTTTWebhook {
	...
	send(options: IFTTTWebhookSendOptions = {}): Promise<Response> {
		...
		let eventName: string | undefined = this.#eventNameDefault;
		...
		return fetch(`https://maker.ifttt.com/trigger/${eventName}${arbitrary ? "/json" : ""}/with/key/${this.#key}`, {
